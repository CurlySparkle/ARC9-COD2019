...
local function doesEntityBreathe(ent)
    return (ent:GetBloodColor() ~= BLOOD_COLOR_MECH and ent:GetBloodColor() ~= DONT_BLEED) or (ent.IsVJBaseSNPC and ent.BloodColor ~= false)
end
...
function ENT:Think()
    ...
    for _, e in pairs(ents.FindInSphere(self:GetPos(), self.GasRadius)) do
        if e:IsPlayer() or e:IsNPC() or e:IsNextBot() then
            if not doesEntityBreathe(e) or not e:IsLineOfSightClear(self:GetPos()) then continue end
